...
public sealed partial class RevitRibbonService(ISettingsService settingsService)
{
    ...
    private void CreatePanels(UIApplication uiApplication)
    {
        ...
        pullButton.AddPushButton<ShowDashboardCommand>("Dashboard").SetAvailabilityController<CommandAlwaysAvailableController>();

        if (!settingsService.ApplicationSettings.UseModifyTab)
        {
            pullButton.AddPushButton<DecomposeSelectionCommand>("Snoop Selection").TryAddShortcuts("Fn12");
        }
        ...
        pullButton.AddPushButton<ShowEventMonitorCommand>("Event monitor").SetAvailabilityController<CommandAlwaysAvailableController>();
